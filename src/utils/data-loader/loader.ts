...
const SOURCE_BUCKET_URL = "https://d2x189otrihntz.cloudfront.net";
...
async function LoadSampleData(statusCallback: (status: StatusUpdate) => void) {
  ...
  if (SOURCE_BUCKET_URL.includes("cloudfront.net")) {
    ...
    const { errors: catErrors, data: catSub } =
    ...
    if (currCategoryCount !== numCategories) {
      ...
      for (const cat of categories) {
        ...
        if (errors !== undefined) {
          ...
          if (
            errors[0].errorType !== "DynamoDB:ConditionalCheckFailedException"
          ) {
            localStatus["categoryStatus"] = "error";
            localStatus[
              "categoryResultText"
            ] = `Error loading categories: ${errors[0].errorType}`;
            break;
          }
        } else {
          categoryCount += 1;
          localStatus["categoryProgress"] =
            (categoryCount / numCategories) * 100;
        }
        statusCallback(localStatus);
      }
      ...
      for (const prod of products) {
        ...
        if (errors !== undefined) {
          ...
          if (
            ...
            localStatus[
              "productResultText"
            ] = `Error loading products: ${errors[0].errorType}`;
            break;
          }
        } else {
          productCount += 1;
    ...
    localStatus["categoryStatus"] = "error";
    localStatus["categoryResultText"] =
      "Make sure you specify the value for SOURCE_BUCKET_URL.";
    localStatus["productStatus"] = "error";
    localStatus["productResultText"] =
      "Make sure you specify the value for SOURCE_BUCKET_URL.";
    statusCallback(localStatus);
